...
export function SprintChallengeCard({ challenge, sprintData, onDelete }: SprintChallengeCardProps) {
  ...
  // Find realized value from sprint data
  const kpiData = sprintData.find(s => s.category === challenge.category);
  const assessorData = kpiData?.assessorBreakdown.find(
    a => a.name === challenge.assessor_name
  );
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between mt-0.5">
            ...
            <span className="text-scale-5">{statusLabel}</span>
          </div>
        </div>
      </div>
    </div>
