...
function getUrgencyIcon(progressPercentage: number, isCompleted: boolean) {
  ...
  return <Flame className="h-3 w-3 lg:h-4 lg:w-4 text-red-400 animate-pulse" />;
}
    ...
    <div className="p-1 lg:p-1.5 bg-card rounded-lg border border-border flex-1 flex flex-col min-h-0 overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[12px] lg:text-[14px] font-semibold text-foreground truncate">
            {label}
          </span>
        </div>
        <span className={cn(
      ...
      <div className="grid grid-cols-3 gap-1 mb-1 text-center text-[10px] lg:text-[10px]">
        <div>
      ...
      {evolution && evolution.difference !== 0 && (
        <div className="flex items-center gap-1 text-[10px] lg:text-[10px] mb-1">
      ...
      {assessorBreakdown.length > 0 && (
        <div className="mt-auto pt-1 border-t border-border/50">
          <span className="text-[10px] lg:text-[10px] text-muted-foreground mb-1 block">
            Falta por Assessor:
          </span>
          <div className="grid grid-cols-3 lg:grid-cols-4 gap-1">
            {assessorBreakdown.map((assessor, idx) => (
              ...
              >
                <span className="text-[10px] lg:text-[12px] font-medium">
                  {assessor.name}
                </span>
                <span className="text-[12px] lg:text-[12px] font-bold">
                  {assessor.achieved 
                    ? "✓" 
      ...
      {isCompleted && assessorBreakdown.length === 0 && (
        <div className="mt-auto text-[10px] lg:text-[10px] text-green-500 font-medium">
